...
#     def forward(self, x):
#         # Only add noise during training and if noise_std is nonzero.
...
class FeatureNoise(nn.Module):
    def __init__(self, noise_std=1, noise_mode='multiplicative'):
        """
        noise_std: Standard deviation of the noise.
        noise_mode: 'additive' for x + noise or 'multiplicative' for x * (1 + noise)
        """
        super(FeatureNoise, self).__init__()
        self.noise_std = noise_std
    ...
    def forward(self, x):
        # Only add noise during training and if noise_std is nonzero.
        if self.training and self.noise_std > 0:
            # For convolutional features, sample noise per channel.
            if x.dim() == 4:  # Assuming x is (batch, channels, height, width)
                noise = torch.randn(x.size(0), x.size(1), 1, 1, device=x.device) * self.noise_std
            else:
                noise = torch.randn_like(x) * self.noise_std
            ...
            if self.noise_mode == 'additive':
                ...
                raise ValueError("Unsupported noise_mode. Choose 'additive' or 'multiplicative'.")
        return x
...
#         # # 给定文本
#         # text = ["a photo of a %s"%item for item in classes_names]
#         # # !!! 多个prompt, avg
#         # # 使用 CLIP 的文本编码器对文本进行编码
#         # text_tokens = clip.tokenize(text).to(device)
#         # with torch.no_grad():
#         #     text_features = model.encode_text(text_tokens)
#         # self.nlpanchor=text_features

#         # Generate multiple prompts per class
#         text_prompts = [
#             [
#                 f"a photo of a {item}",
#                 f"an image of a {item}",
#                 f"a depiction of a {item}",
#                 f"a representation of a {item}",
#                 f"a {item}",
#                 f"a close-up of a {item}",
#                 f"a detailed image of a {item}",
#                 f"a snapshot of a {item}",
#                 f"a clear picture of a {item}"
#             ]
...
#         self.maplayers=nn.ModuleList([nn.Linear(self.featurizer.n_outputs,len(self.nlpanchor[0])).to(device) for i in range(self.num_classes)])
...
#             )
#         else:
#             raise NotImplementedError('please provide mapsty')

#     def align_loss(self,fea,y):
#         #ot的匹配,用线性层模仿ot的映射
#         losses=torch.zeros(len(y)).to(y.device)
#         # print(self.maplayers[y[0]](fea[0]).shape)
#         for i in range(len(fea)):
#             # losses[i]=torch.sum(torch.square(self.maplayers[y[i]](fea[i])-self.nlpanchor[y[i]]))
#             losses[i]=-F.cosine_similarity(self.maplayers[y[i]](fea[i]),self.nlpanchor[y[i]],dim=0)
#         # print(losses)
#         # print(-losses)
#         weights=F.softmax((-losses.detach()*10)).detach()
#         # weights=F.normalize(2-losses.detach(),p=1,dim=0)
#         return torch.sum(losses*weights),weights

#     # !!! EMA update (microsoft USB)
...
#         loss = alnloss*self.hparams['lambda']+torch.sum(F.cross_entropy(self.classifier(all_fea), all_y,reduction='none')*weights)
#         self.optimizer.zero_grad()
#         self.optimizer1.zero_grad()
#         loss.backward()
...
#         # print(weights)
#         # print(torch.sum(F.cross_entropy(self.classifier(all_fea), all_y,reduction='none')*weights))
#         # print(alnloss*self.hparams['lambda'])
#         # exit()
...
# TODO: change cosine similarity to Weak OT begin
def _sinkhorn_plan_1d_hist(a, b, eps=0.05, n_iters=50, device=None):
    D = a.shape[0]
    if device is None:
        device = a.device
...
def _weak_ot_cost_1d_hist(a, b, eps=0.05, n_iters=50, device=None):
    """
    cost: sum_i a_i * ( i - E_bary[i] )^2
    E_bary[i] = (1/a_i) * sum_j pi_{ij} * j
    """
    D = a.shape[0]
    if device is None:
        device = a.device
    ...
    pi, a = _sinkhorn_plan_1d_hist(a, b, eps=eps, n_iters=n_iters, device=device)  # (D,D)
    a_safe = a.clamp_min(1e-12)
    bary = (pi @ idx) / a_safe  # (D,)
    cost = torch.sum(a * (idx - bary) ** 2)
    return cost
# TODO: change cosine similarity to Weak OT end
...
class NLPGERM(ERM):
    ...
    def align_loss(self, fea, y):
        ...
        for i in range(N):
            ...
            wot = _weak_ot_cost_1d_hist(pa, pb, eps=eps, n_iters=iters, device=device)
            # wot = wot.clamp_min(0.0)
            losses[i] = wot
...
class NLPGERM_NoNLP(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(NLPGERM_NoNLP, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.num_classes = num_classes
        self.ma_start_iter = 100
        self.global_iter = 0
        self.ma_count = 0
...
class NLPGERM_L2(ERM):

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(NLPGERM_L2, self).__init__(input_shape, num_classes, num_domains,
                                      hparams)
        self.num_classes = num_classes
    ...
    def align_loss(self, fea, y):
        ...
        for i in range(len(fea)):
            # losses[i] = -F.cosine_similarity(self.maplayers[y[i]](fea[i]), self.nlpanchor[y[i]], dim=0)
            losses[i] = torch.norm(self.maplayers[y[i]](fea[i]) - self.nlpanchor[y[i]], p=2)
        weights = F.softmax((-losses.detach() * 10)).detach()
        return torch.sum(losses * weights), weights
...
class NLPGERM_NoOTLoss(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(NLPGERM_NoOTLoss, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.num_classes = num_classes
        self.network_ma = copy.deepcopy(self.network)
        self.network_ma.eval()
        
        # Retain NLP anchors
        self.nlpanchor = None  # Will be set later via set_nlp_anchor
    ...
    def set_nlp_anchor(self, dataset):
        ...
        self.nlpanchor = torch.stack(all_text_features)  # Keep NLP anchors

    def update(self, minibatches, unlabeled=None):
        """
        Update the main model without OT loss.
        """
        all_x = torch.cat([x for x, y in minibatches])
        all_y = torch.cat([y for x, y in minibatches])
        all_fea = self.featurizer(all_x)

        # Use NLP anchors in classification by projecting features directly
        similarity_scores = torch.mm(all_fea, self.nlpanchor.T)  # Dot product similarity
        loss = F.cross_entropy(similarity_scores, all_y)  # Classification loss
