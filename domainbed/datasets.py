...
class DomainNoisyImageFolder(ImageFolder):
    """
    Extends ImageFolder to attach a domain label to each sample.
    With probability `domain_noise`, the domain label is randomly flipped
    to one of the other domain indices.
    """
    def __init__(self, root, transform, domain_noise=0, domain_id=0, num_domains=1):
        super(DomainNoisyImageFolder, self).__init__(root, transform=transform)
        self.domain_noise = domain_noise
        self.domain_id = domain_id
        self.num_domains = num_domains
        # Initially assign all samples to the current domain.
        self.domain_labels = [domain_id] * len(self.samples)
        # Inject noise: for each sample, with probability domain_noise,
        # assign a random different domain.
        if self.domain_noise > 0:
            for idx in range(len(self.samples)):
                if random.random() < self.domain_noise:
                    possible_domains = list(range(num_domains))
                    possible_domains.remove(domain_id)
                    self.domain_labels[idx] = random.choice(possible_domains)

    def __getitem__(self, index):
        # Standard image and class label from ImageFolder.
        img, target = super(DomainNoisyImageFolder, self).__getitem__(index)
        # # Also return the (possibly noisy) domain label.
        # domain = self.domain_labels[index]
        return img, target
...
class MultipleEnvironmentImageFolder(MultipleDomainDataset):
    def __init__(self, root, test_envs, augment, hparams):
        ...
        self.input_noise = hparams.get('input_noise', 0)  # Default to 0 if not provided.
        self.domain_noise = hparams.get('domain_noise', 0)  # Default to 0 if not provided.
        ...
        # If input_noise is set (and > 0), define a noise transform.
        if self.input_noise and self.input_noise > 0:
            noise_transform = AddGaussianNoise(0, self.input_noise)
            transform = transforms.Compose([
                transforms.Resize((224,224)),
                transforms.ToTensor(),
                noise_transform,           # Add noise after conversion to tensor.
                normalize,
            ])
            augment_transform = transforms.Compose([
                ...
                transforms.ToTensor(),
                noise_transform,           # Add noise here as well.
                normalize,
            ])
        else:
            # No input noise; use the original transforms.
            transform = transforms.Compose([
                transforms.Resize((224,224)),
                transforms.ToTensor(),
                normalize,
        ...
        for i, environment in enumerate(environments):
            ...
            if self.domain_noise is not None and self.domain_noise > 0:
                env_dataset = DomainNoisyImageFolder(
                    path,
                    transform=env_transform,
                    domain_noise=self.domain_noise,
                    domain_id=i,
                    num_domains=len(environments)
                )
            else:
...
class DomainNet(MultipleEnvironmentImageFolder):
    ...
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "domain_net_subset/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)
...
class WILDSWaterbirds(WILDSDataset):
    ...
    def __init__(self, root, test_envs, hparams):
        ...
        self.classes = ['landbird on land', 'waterbird on land', 'landbird on water', 'waterbird on water']
        

# class WILDSWaterbirds(WILDSDataset):
#     # Mapping numerical groups to meaningful NLP descriptions
#     GROUP_DESCRIPTIONS = {
#         0: "a landbird on land",
#         1: "a waterbird on land",
#         2: "a landbird on water",
#         3: "a waterbird on water"
#     }
...
#         dataset.classes = [
#         [self.GROUP_DESCRIPTIONS[int(g)]] for g in dataset._metadata_array.squeeze().tolist()]
...
#         self.dataset = dataset
#         self.classes = dataset.classes  # Ensure NLPGERM gets correct class names
...
class WILDSCamelyon(WILDSDataset):
    ...
    def __init__(self, root, test_envs, hparams):
        ...
        # from wilds.datasets.iwildcam_dataset
...
class WILDSFMoW(WILDSDataset):
    ...
    def __init__(self, root, test_envs, hparams):
        ...
        # dataset = FMoWDataset(root_dir=root, download=False)
        full_dataset = FMoWDataset(root_dir=root, download=False)
        # Assume full_dataset is already loaded as an instance of FMoWDataset.
        train_subset = full_dataset.get_subset(split='train', frac=0.1, transform=None)
        val_subset   = full_dataset.get_subset(split='val',   frac=0.1, transform=None)
        test_subset  = full_dataset.get_subset(split='test',  frac=0.1, transform=None)
        combined_indices = list(train_subset.indices) + list(val_subset.indices) + list(test_subset.indices)
        dataset = WILDSSubset(full_dataset, combined_indices, transform=None)
