...
class BestValidationSelectionMethod(SelectionMethod):
    """Selects the model with the highest validation accuracy across all environments."""
    name = "Best Validation Accuracy Selection"

    @classmethod
    def run_acc(self, run_records):
        """Find the record with the highest validation accuracy across all test environments."""
        if not len(run_records):
            return None

        # Identify validation accuracy keys: env*_out_acc (excluding noisy/train metrics)
        val_acc_keys = [key for key in run_records[0].keys() if key.endswith('_out_acc')]

        if not val_acc_keys:
            print("No validation accuracy keys found!")
            return None

        # Find the record with the highest validation accuracy
        best_record = max(run_records, key=lambda r: max(r[key] for key in val_acc_keys))
        best_val_acc = max(best_record[key] for key in val_acc_keys)

        return {
            'val_acc': best_val_acc,
            'record_dir': best_record['args']['output_dir']  # Directory of the best model
        }
