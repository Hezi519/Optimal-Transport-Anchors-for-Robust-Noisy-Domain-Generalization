...
def plot_data(data, dataset, train_data=None, val_data=None):
    ...
    plot_dir = os.path.join('figures', "comb", dataset)
    ...
    for alg in data.keys():
        ...
        num_seeds = data[alg].shape[0]
        
        for i in range(num_seeds):
            color = colors[i % len(colors)]  # assign a distinct color per seed
            trial_i = data[alg][i]
            
            if trial_i.ndim == 2:
                # If each seed has multiple groups, plot each group using the same seed color
                for j in range(trial_i.shape[0]):
                    trajectory = trial_i[j]
                    plt.plot(trajectory, label=f'seed{i}_group{j}', linewidth=3, color=color)
            else:
                plt.plot(trial_i, label=f'seed{i}', linewidth=3, color=color)
                # plt.plot(trial_i, linewidth=3, color=color)
            ...
            if val_data is not None:
                ...
                if val_trial_i.ndim == 2:
                    for j in range(val_trial_i.shape[0]):
                        ...
                        plt.plot(trajectory, label=f'{alg}_val_seed{i}_group{j}', linewidth=3, color=color)
                else:
                    plt.plot(val_trial_i, label=f'{alg}_val_seed{i}', linewidth=3, color=color)
            ...
            if train_data is not None:
                ...
                if train_trial_i.ndim == 2:
                    for j in range(train_trial_i.shape[0]):
                        ...
                        plt.plot(trajectory, label=f'{alg}_train_seed{i}_group{j}', linewidth=3, color=color)
                else:
                    plt.plot(train_trial_i, label=f'{alg}_train_seed{i}', linewidth=3, color=color)
        ...
        plt.legend(fontsize=20)  # Adjust legend font size as needed
        
        plt.savefig(os.path.join(plot_dir, f'{alg}_trajectories.png'))
