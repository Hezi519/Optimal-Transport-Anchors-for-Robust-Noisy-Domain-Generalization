...
if __name__ == "__main__":
    ...
    parser.add_argument("--get_best", default=False)
    ...
    # print(records[0].keys())  #['args', 'env0_noisy_acc', 'env0_out_acc', 'env1_noisy_acc', 'env1_out_acc', 'env2_noisy_acc', 'env2_out_acc', 'env3_in_acc', 'env3_out_acc', 'epoch', 'hparams', 'loss', 'mem_gb', 'step', 'step_time']
    
    
    # Independent execution for --get_best
    if args.get_best:
        best_model_result = model_selection.BestValidationSelectionMethod.run_acc(records)
        if best_model_result and best_model_result["record_dir"]:
            print(best_model_result["record_dir"])  # Print only directory name
        else:
            print("No best model found!")
        sys.exit(0)  # Exit early to avoid further execution
