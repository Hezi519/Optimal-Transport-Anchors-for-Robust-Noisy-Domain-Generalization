...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join(args.output_dir,'done')):
        exit()
    ...
    # print("dataset info:")
    # print(datasets.get_dataset_class("VLCS"))
    # print(datasets.get_dataset_class("VLCS").ENVIRONMENTS)
    # print(datasets.num_environments("WILDSWaterbirds"))
    # print(dataset.dataset._metadata_array)
    ...
    if torch.cuda.mem_get_info()[1]/1e9 > 20:
        print("Detected large GPU memory. Setting num_workers to 2")
        dataset.N_WORKERS = 0
    ...
    train_loaders = [InfiniteDataLoader(
        ...
        for i, (env, env_weights) in enumerate(in_splits)
        if i not in args.test_envs]
    print("Finished Train Data Loader Initialization!")
    ...
    print("Eval Data Updated!")
    algorithm_class = algorithms.get_algorithm_class(args.algorithm)
    # if 'mixmatch' in args.algorithm.lower():
    #     algorithm = algorithm_class(dataset.input_shape, dataset.num_classes,
    #         len(dataset) - len(args.test_envs),hparams hparams)
    # else:
    algorithm = algorithm_class(dataset.input_shape, dataset.num_classes,
    ...
    # print(f"Steps per epch: {steps_per_epoch}")

    n_steps = args.steps or dataset.N_STEPS
    checkpoint_freq = args.checkpoint_freq or dataset.CHECKPOINT_FREQ
    # print(f"n steps: {n_steps}")
    # print(int(n_steps/steps_per_epoch/10))
    ...
    if isinstance(algorithm, algorithms.NLPGERM) or args.algorithm=='NLPGERM_NoWeight':
        print(args.dataset)
    ...
    for step in range(start_step, n_steps):
        ...
        if (isinstance(algorithm, algorithms.NLPGERM) or args.algorithm=='NLPGERM_NoWeight') and (hparams['mapsty']=='itera') and ((int(step / steps_per_epoch)) % (int(n_steps/steps_per_epoch/hparams['iter_freq']))==0):
